 ...
 */
typedef struct {
  void *A; // this is the array used to store the heap nodes
  ...
  unsigned int *key_pos; // reports the position of the key of a node
  unsigned int *rev_pos; // reports the node corresponding to a given position
  unsigned int real_dim; // first available position where a new node can be inserted
} binheap_type;
